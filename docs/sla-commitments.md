...
- **Target:** Respond to the reporter within five business days via the same
  channel used for submission (GitHub PVR or
  [security@cloudsecurityalliance.org](mailto:security@cloudsecurityalliance.org)).
- **Purpose:** Confirms receipt, shares the tracking identifier, and communicates
  the next expected milestone.

## Status updates

- **Target:** Provide a substantive update at least every 30 days while the
  issue remains open.
- **Purpose:** Keeps the reporter informed about investigation, remediation
  progress, or dependencies that may affect timing.

## Remediation or disclosure

- **Target:** Resolve, mitigate, or coordinate publication within 90 days of
  acknowledgment.
- **Negotiation:** If the fix or publication cannot complete within 90 days, CSA
  and the reporter agree on a new timeline and record the decision in the case
  log.
